      ...
      xhr.onload = function() {
        if(xhr.status == 200) {
          ...
          for(;dayCount < 5;){
            ...
            tempStr += '<div class="weather-widget" data-code="' + forecast[dayCount].code + '">';
            tempStr += '<time datetime="' + forecast[dayCount].date + '">' + forecast[dayCount].date + '</time>';
            tempStr += '<div class="weather-widget__condition-now">' + forecast[dayCount].text + '</div>';
            tempStr += '<div class="weather-widget__temp-now"> max ' + forecast[dayCount].high + ' °' + units.temperature + '</div>';
            tempStr += '<div class="weather-widget__temp-now"> min ' + forecast[dayCount].low + ' °' + units.temperature + '</div>';
            tempStr += '</div>';
            dayCount++;
          }
